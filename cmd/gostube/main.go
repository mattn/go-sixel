...
var (
	...
	fFormat = flag.String("format", "bv*[height<=480]+ba/b[height<=480]/b", "yt-dlp format selector")
)
...
func main() {
	flag.Usage = func() {
		fmt.Println("Usage of " + os.Args[0] + ": gostube [options] url")
		flag.PrintDefaults()
	...
	videoURL, audioURL, err := resolveStreams(flag.Arg(0))
	if err != nil {
		log.Fatal(err)
	}
	meta, err := probeVideo(videoURL)
	if err != nil {
		log.Fatal(err)
	}
	if !*fMute {
		if p, err := exec.LookPath("ffplay"); err == nil {
			...
			fmt.Fprintln(os.Stderr, "gostube: ffplay not found, playing without audio")
		}
	}
	withAudio := audioPlayer != "" && (audioURL != videoURL || meta.HasAudio)
	...
	var playErr error
	for {
		next, err := play(ctx, videoURL, audioURL, width, height, fps, offset, withAudio, keys)
		if err != nil {
			if ctx.Err() == nil {
				playErr = err
			}
...
// resolveStreams asks yt-dlp for direct media URLs. It returns separate
// video and audio URLs when the selected format pairs distinct streams,
// or the same URL twice for a muxed format.
func resolveStreams(url string) (string, string, error) {
	cmd := exec.Command("yt-dlp", "--no-playlist", "-g", "-f", *fFormat, url)
	...
	switch len(urls) {
	case 1:
		return urls[0], urls[0], nil
	case 2:
		return urls[0], urls[1], nil
	default:
		return "", "", fmt.Errorf("yt-dlp returned %d stream URLs, expected 1 or 2", len(urls))
	}
}
...
func play(ctx context.Context, videoURL, audioURL string, width, height int, fps, offset float64, withAudio bool, keys <-chan float64) (float64, error) {
	...
	cmd := ffmpegCommand(pctx, videoURL, audioURL, width, height, fps, offset, audioSink)
	...
	if withAudio {
		...
		if audio := startAudio(pctx, audioR); audio != nil {
			...
			fmt.Fprintln(os.Stderr, "gostube: ffplay failed to start, playing without audio")
		}
		audioR.Close()
	}
...
func ffmpegCommand(ctx context.Context, videoURL, audioURL string, width, height int, fps, offset float64, audioSink string) *exec.Cmd {
	withAudio := audioSink != ""
	// Protocol and seek options apply per input, so repeat them for each -i.
	netArgs := []string{
		"-reconnect", "1",
		"-reconnect_streamed", "1",
		"-reconnect_delay_max", "5",
		// Force real HTTP range requests when seeking. Without this,
		// ffmpeg "soft-seeks" by draining the rest of the throttled
		// stream, so -ss on a seek never starts playing.
		"-short_seek_size", "1",
	}
	var seekArgs []string
	if offset > 0 {
		seekArgs = []string{"-ss", strconv.FormatFloat(offset, 'f', 3, 64)}
	}
	...
	audioInput := "0"
	if withAudio && audioURL != videoURL {
		audioInput = "1"
		args = append(args, netArgs...)
		args = append(args, seekArgs...)
...
func probeVideo(url string) (*videoMeta, error) {
	cmd := exec.Command(
		"ffprobe",
		"-v", "error",
	...
	if meta.Width == 0 {
		return nil, fmt.Errorf("no video stream found in %q", url)
	}
	return &meta, nil
}
